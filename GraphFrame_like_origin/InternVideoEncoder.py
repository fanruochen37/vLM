...
class InternVideoEncoder:
    def __init__(self, model_path="/home/teacher_tang/Video-Holmes/InternVL_2_5_HiCo_R64_series/InternVL_2_5_HiCo_R64"):
    ...
    def encode_full_video(self, video_path, fps=1, local_num_frames=4):
        ...
        with torch.no_grad():
            video_features = self.encoder.extract_feature(pixel_values.to(torch.bfloat16).cuda()) 
            print(video_features.shape)
            
        # 时序建模：保持 64 tokens 独立，仅在时间轴平滑
        video_features = self.temporal_modeling(video_features)
        return video_features, frame_indices, fps
